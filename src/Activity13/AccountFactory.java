    ...
    public static IAccount createAccount(String type, String accNum, String name, int age, double balance, String status, String pin, int tenureYears) {
        //throw new UnsupportedOperationException("TODO: implement tenure-aware AccountFactory.createAccount (Activity 13.2, Step 2)");
         ...
         return switch(type.toUpperCase()){
            case "SAVINGS"->new SavingsAccount(accNum, name, age, balance, status, pin, tenureYears);
            case "CURRENT"->new CurrentAccount(accNum, name, age, balance, status, pin, 25000.0);
            case "FIXED-DEPOSIT","FD"->new FixedDepositAccount(accNum, name, age, balance, status, pin, 12, 6.5);
            case "SALARY"->new SalaryAccount(accNum, name, age, balance, status, pin, "TechCorp");
