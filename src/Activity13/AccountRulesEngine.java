...
public class AccountRulesEngine {
    ...
    private static final Map<String,Double> SAVINGS_MINIMUM_BALANCE=new HashMap<>();
    private static final Map<String,Double> SAVINGS_INTEREST_RATE=new HashMap<>();
    static{
        SAVINGS_MINIMUM_BALANCE.put("NEW",10000.0);
        SAVINGS_MINIMUM_BALANCE.put("STANDARD",7500.0);
        SAVINGS_MINIMUM_BALANCE.put("PREMIUM",5000.0);
        SAVINGS_MINIMUM_BALANCE.put("PRIVILEGE",2500.0);
    ...
    public static double getSavingsMinBalance(int tenureYears) {
        ...
        String savingsBucket=getSavingsBucket(tenureYears);
        /*return switch (savingsBucket) {
            case "PRIVILEGE" -> SAVINGS_MINIMUM_BALANCE.get("PRIVILEGE");
            case "PREMIUM" -> SAVINGS_MINIMUM_BALANCE.get("PREMIUM");
            case "STANDARD" -> SAVINGS_MINIMUM_BALANCE.get("STANDARD");
            case "NEW" -> SAVINGS_MINIMUM_BALANCE.get("NEW");
            default -> 10000.0;
        };*/
        return SAVINGS_MINIMUM_BALANCE.getOrDefault(savingsBucket,10000.0);
    ...
    public static double getSavingsInterestRate(int tenureYears) {
        ...
        String savingsBucket=getSavingsBucket(tenureYears);
        return SAVINGS_INTEREST_RATE.getOrDefault(savingsBucket,2.70);
